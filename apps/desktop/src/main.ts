...
const APP_USER_MODEL_ID = isDevelopment ? "com.t3m0.app.dev" : "com.t3m0.app";
const LINUX_DESKTOP_ENTRY_NAME = isDevelopment ? "t3m0-dev.desktop" : "t3m0.desktop";
const LINUX_WM_CLASS = isDevelopment ? "t3m0-dev" : "t3m0";
const USER_DATA_DIR_NAME = isDevelopment ? "t3m0-dev" : "t3m0";
const LEGACY_USER_DATA_DIR_NAME = isDevelopment ? "T3M0 (Dev)" : "T3M0 (Alpha)";
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("T3M0 failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `T3M0 ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
 ...
 * parentheses (e.g. `~/.config/T3M0 (Alpha)` on Linux). This is
 * unfriendly for shell usage and violates Linux naming conventions.
 *
 * We override it to a clean lowercase name (`t3m0`). If the legacy
